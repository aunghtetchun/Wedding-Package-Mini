...
@section('content')
    ...
    @component("component.breadcrumb",["data"=>[
        "Wedding Package" => route("wedding_package.index"),
    ]])
        @slot("last") Edit Wedding Package @endslot
    @endcomponent
    ...
    <div class="row">
        <div class="col-12 col-md-6">
            @component("component.card")
                ...
                @slot('title') Edit Wedding Package @endslot
                @slot('button')
                ...
                @endslot
                @slot('body')
                        <form action="{{ route('wedding_package.update',$weddingPackage->id) }}" method="post" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div class="form-group">
                                <label for="title">Title</label>
                                <input type="text" class="form-control @error('title') is-invalid @enderror" name="title" id="title" value="{{ $weddingPackage->title }}" placeholder="Title">
                                @error('title')
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" class="form-control @error('price') is-invalid @enderror" name="price" id="price" value="{{ $weddingPackage->price }}" placeholder="Price">
                                @error('price')
                                <small class="invalid-feedback font-weight-bold" role="alert">
                                    {{ $message }}
                                </small>
                            ...
                            <div class="form-group">
                                ...
                                <textarea class="form-control @error('street') is-invalid @enderror" name="description" id="description">{!!  $weddingPackage->description !!}</textarea>
                                @error('description')
                                <small class="invalid-feedback font-weight-bold" role="alert">
                                    {{ $message }}
                                </small>
                            ...
                            <button type="submit" class="btn btn-primary " ><i class="fas fa-plus-square mr-1"></i> Update Wedding Package</button>
                        </form>
        ...
        <div class="col-12 col-md-6">
            @component("component.card")
                @slot('icon') <i class="feather-file text-primary"></i> @endslot
                @slot('title') Wedding Package Photo @endslot
                @slot('button')
...
{{--                                    <th scope="col">Control</th>--}}
                                ...
                                <tbody>
                                @foreach($weddingPackage->getPackageDetail as $pd)
                                    <tr>
                                        <td>{{ $pd->id }}</td>
                                        <td>{{ $pd->title }}
                                ...
                                <tr>
                                    ...
                                    <td >
                                        {{ $weddingPackage->getPackageDetail->sum('cost') }}
                                    </td>
                                </tr>
                                </tbody>
                            </table>
